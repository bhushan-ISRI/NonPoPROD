...
const DEFAULT_VOUCHING_VIEW_PATH = "/Vouching";
const DEFAULT_VOUCHING_EDIT_PATH = "/Vouching";
...
/** ---------------- Component ---------------- */
const PendingforVouching: React.FC<PendingforVouchingProps> = ({
  currentSPContext,
  listTitle = "NonPO",
  title = "Pending for Vouching",
  vouchingViewPath = DEFAULT_VOUCHING_VIEW_PATH,
  vouchingEditPath = DEFAULT_VOUCHING_EDIT_PATH,
  ...props
}) => {
  useFullscreenForm();
  ...
  const [rows, setRows] = useState<PendingVouchRow[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const select =
          "Id,Title,RequestDate,EmployeeName,CurrentApprover/Id,CurrentApprover/EMail,CurrentApprover/Title,VendorName,InvoiceNumber,Totalamount,Amount,Status,VendorCode/VendorCode,VendorCode/Id,VendorCode/Title";
        const expand = "VendorCode,CurrentApprover";
        const orderBy = { column: "Id", isAscending: false };

        // Filter: Pending for Vouching
        const filter = `Status eq 'Pending for Vouching'`;
        ...
        const mapped: PendingVouchRow[] = items.map((it: any) => ({
          _spId: it?.Id,
          requestNo: it?.Title || "",
          ...
          amount:
            it?.Totalamount !== undefined && it?.Totalamount !== null && it?.Totalamount !== ""
              ...
              : safeNum(it?.Amount),
          status: it?.Status || "Pending for Vouching",
        }));
        let filteredmapped = mapped.filter((m) => (m.CurrentApproverEmail === props.userEmail))
        setRows(mapped);
      } catch (e: any) {
        console.error("[PendingforVouching] fetch error:", e);
        setError(e?.message || "Failed to load Vouching data");
      } finally {
  ...
  /**  View  */
  const handleView = (row: PendingVouchRow) => {
    if (!row?._spId) {
      alert("Item ID missing for this row.");
      return;
    }
    navigate(`${vouchingViewPath}?id=${row._spId}&mode=view`, {
      state: { from: "/APTeam/PendingforVouching", fullscreen: true },
    });
  };

  /**  Edit */
  const handleEdit = (row: PendingVouchRow) => {
    if (!row?._spId) {
      alert("Item ID missing for this row.");
      return;
    }
    navigate(`${vouchingEditPath}?id=${row._spId}&mode=edit`, {
      state: { from: "/APTeam/PendingforVouching", fullscreen: true },
    });
  };
                ...
                {!loading && totalItems === 0 ? (
                  ...
                  paged.map((row) => (
                    <tr key={row._spId ?? `${row.requestNo}-${row.invoiceNo}`}>
                      ...
                      <td>{row.status || "Pending for Vouching"}</td>
