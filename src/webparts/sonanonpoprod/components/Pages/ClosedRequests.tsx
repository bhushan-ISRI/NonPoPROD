...
const DEFAULT_VIEW_FORM_PATH = "/ClosedRequests";
...
/** ---------------- Component ---------------- */
const ClosedRequests: React.FC<ClosedRequestsProps> = ({
  currentSPContext,
  listTitle = "NonPO",
  title = "Closed Requests",
  viewFormPath = DEFAULT_VIEW_FORM_PATH,
}) => {
  useFullscreenForm();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const select =
          "Id,Title,RequestDate,EmployeeName,VendorName,VendorCode/VendorCode,VendorCode/Id,InvoiceNumber,Totalamount,Amount,Status,VendorCode/Title";
        const expand = "VendorCode";
        const orderBy = { column: "Id", isAscending: false };


        const filter = `Status eq 'Closed'`;
        ...
        const mapped: ClosedRow[] = items.map((it: any) => ({
          _spId: it?.Id,
          requestNo: it?.Title || "",
          ...
          amount:
            it?.Totalamount !== undefined && it?.Totalamount !== null && it?.Totalamount !== ""
              ...
              : safeNum(it?.Amount),
          status: it?.Status || "Closed",
        }));

        setRows(mapped);
      } catch (e: any) {
        console.error("[ClosedRequests] fetch error:", e);
        setError(e?.message || "Failed to load Closed requests");
      } finally {
        setLoading(false);
      }
    })();
  ...
  const handleView = (row: ClosedRow) => {
    if (!row?._spId) {
      alert("Item ID missing for this row.");
      return;
    }
    navigate(`${viewFormPath}?id=${row._spId}&mode=view`, {
