...
const InitiatorLanding: React.FC<InitiatorLandingProps> = ({
  listTitle = "NonPO",
  onAddNon,
  onViewRow,
  title = "NON PO Initiator",
  ...props
}) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const userId = await resolveCurrentUserId(props.currentSPContext);
        if (!userId) {
          throw new Error(
        ...
        const select =
          "Id,Title,RequestDate,VendorName,InvoiceNumber,Totalamount,Amount,Status,VendorCode/VendorCode,VendorCode/Id,VendorCode/Title";
        const expand = "VendorCode";
        const filter = `AuthorId eq ${userId}`;
  ...
  const handleAddNon = () => {
    ...
    else navigate("/NonPoRequest?mode=new"); // create mode
  };

  const handleView = (row: NonPoRow) => {
    if (onViewRow) return onViewRow(row);
    if (!row?._spId) return;
    navigate(`/NonPoRequest?id=${row._spId}&mode=view`, { state: { from: "/InitiatorLanding", fullscreen: true } });
  };

  const handleEdit = (row: NonPoRow) => {
    if (!row?._spId) return;
    navigate(`/NonPoRequest?id=${row._spId}&mode=edit`, { state: { from: "/InitiatorLanding", fullscreen: true } });
  };
  ...
  return (
    <div>
      ...
      <div className='col-md-12 px-2 d-flex justify-content-between align-items-center flex-wrap' style={{margin: "5px"}}>
        ...
        <div className='Dashbaordcreatebutton' style={{paddingRight :"10px"}}>
          <a className='create-button' onClick={handleAddNon} >Add Request</a>
        </div>
      </div>
                      ...
                      <tr key={row._spId ?? `${row.requestNo}-${row.invoiceNo}`}>
                        <td>
                          <div style={{ display: "flex", gap: 8, border: "0px solid #f8f3f3" }}>
                            ...
                            {row.status === "Send Back" && (
                              <Link to={`/ViewRequestSendBack/${row._spId}`}>
