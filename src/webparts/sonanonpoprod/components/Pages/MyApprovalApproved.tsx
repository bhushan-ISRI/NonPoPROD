...
export const MyApprovalApproved = (props: ISonanonpoprodProps) => {
    ...
    const loadApproveItems = async () => {
        ...
        const parentItems = await spCrudOps.getData(
            "NonPO",
            "ID,Title,RequestDate,EmployeeName,ApprovalMatrix,VendorCode/VendorCode,WorkflowHistory,Email,ContactNo,Division,Department,Location,RM,HOD,EmployeeStatus,ApprovalMatrix,VendorName,InvoiceNumber,InvoiceDate,Basicamount,GST,OtherCharges,Totalamount,WorkflowHistory,Status,PaymentRemarks,ApprovalRemarks,VendorCode/Id,VendorCode/Title,NatureofExpense/Title,NatureofExpense/ExpenseType,CurrentApprover/ID,CurrentApprover/EMail,CurrentApprover/Title",
        ...
        parentItems.forEach((item: { ApprovalMatrix: string }) => {
            if (item.ApprovalMatrix) {
                let parsedApprovalMatrix = JSON.parse(item.ApprovalMatrix);

                const filtered = parsedApprovalMatrix.filter((x: any) =>
                    String(x.UserID) === String(currentUser.Id) &&
                    x.Status === "Approved"
                );

                filtered.forEach((approval: any) => {
                    tableData.push({
                        ...item,          // parent item fields
        ...
        const uniqueData = Array.from(
            new Map(tableData.map(item => [item.ID, item])).values()
        );

        console.log(uniqueData)
        setApprovedData(uniqueData)
